...
var getFiles = async (path, depth = 0) => {
  ...
  if (depth) {
    for (const folder of folders)
      files.push(
        ...(await getFiles(`${(0, import_path.resolve)(path, folder.name)}/`, depth - 1)).map((a) => ({
          ...a,
          folder: folder.name
        }))
      );
...
var isEsm = (import_meta.url || "").endsWith(".mjs");
var importFile = async (file, outputError = true) => {
  ...
  const importPath = isWindows && isEsm ? (0, import_url.pathToFileURL)(file).href : file;
...
`);
  ...
  const outputPathParts = (0, import_path5.resolve)(outputPath, locale).replace((0, import_path5.resolve)(), "").split(import_path5.sep).filter(import_typesafe_utils13.isTruthy);
