...
var require_isUndefined = __commonJS({
  "../../node_modules/.pnpm/typesafe-utils@1.16.2/node_modules/typesafe-utils/lib/cjs/isUndefined/isUndefined.js"(exports) {
    ...
    var isNotUndefined2 = (value) => value !== void 0;
...
var require_logical = __commonJS({
  "../../node_modules/.pnpm/typesafe-utils@1.16.2/node_modules/typesafe-utils/lib/cjs/logical/logical.js"(exports) {
    ...
    exports.or = or;
    var not2 = (filterFn) => (value, i, array) => !filterFn(value, i, array);
    exports.not = not2;
  }
});
...
var require_uniqueArray = __commonJS({
  "../../node_modules/.pnpm/typesafe-utils@1.16.2/node_modules/typesafe-utils/lib/cjs/uniqueArray/uniqueArray.js"(exports) {
    ...
    var uniqueArray3 = (array) => Array.from(new Set(array));
    exports.uniqueArray = uniqueArray3;
  }
});
    ...
    function resolve7(compile, root, ref) {
      ...
      if (typeof refVal == "string") {
        if (this._refs[refVal])
          ...
          return resolve7.call(this, compile, root, refVal);
      }
  ...
  "../../node_modules/.pnpm/readdirp@3.6.0/node_modules/readdirp/index.js"(exports, module) {
    ...
    var readdirpPromise = (root, options = {}) => {
      return new Promise((resolve7, reject) => {
        const files = [];
        readdirp(root, options).on("data", (entry) => files.push(entry)).on("end", () => resolve7(files)).on("error", (error) => reject(error));
      });
    };
...
var require_sync = __commonJS({
  "../../node_modules/.pnpm/tiny-glob@0.2.9/node_modules/tiny-glob/sync.js"(exports, module) {
    ...
    var { join: join4, resolve: resolve7, relative } = __require("path");
    var isHidden = /(^|[\\\/])\.[^\\\/\.]/g;
    var CACHE = {};
    function walk(output, prefix, lexer, opts, dirname2 = "", level = 0) {
      const rgx = lexer.segments[level];
      const dir = resolve7(opts.cwd, prefix, dirname2);
    ...
    module.exports = function(str, opts = {}) {
      ...
      if (!glob2.isGlob) {
        try {
          let resolved = resolve7(opts.cwd, str);
      ...
      return opts.absolute ? matches.map((x) => resolve7(opts.cwd, x)) : matches;
    };
  }
});
...
var getFiles = async (path, depth = 0) => {
...
var isEsm = (import.meta.url || "").endsWith(".mjs");
...
var getConfigWithDefaultValues = async (config, shouldReadConfig = true) => ({
  baseLocale: "en",
...
var getUniqueFormatters = (parsedTranslations) => {
  ...
  flattenToParsedResultEntry(parsedTranslations).forEach((parsedResult) => {
    ...
    args.forEach(
      ({ key, formatters }) => (formatters || []).filter((formatter) => !formatter.startsWith("{")).forEach((formatter) => map[formatter] = [...map[formatter] || [], ...types[key]?.types || []])
    );
  });
  return Object.entries(map).sort((0, import_typesafe_utils6.sortStringPropertyASC)("0"));
};
var createFormattersType = (parsedTranslations) => {
  ...
  return `export type Formatters = ${wrapObjectType(
    ...
    () => mapToString(
      formatters,
      ([key, types]) => `
	${wrapObjectKeyIfNeeded(key)}: (value: ${(0, import_typesafe_utils6.uniqueArray)(types).join(" | ")}) => unknown`
    )
  )}`;
};
...
var validateNamespaces = (translations, namespaces) => {
  ...
  namespaces.forEach((namespace) => {
    if ((0, import_typesafe_utils7.isString)(translations[namespace])) {
...
var partsAsStringWithoutTypes = (parts) => parts.map(partAsStringWithoutTypes).join("");
var partAsStringWithoutTypes = (part) => {
  if (typeof part === "string") {
    return part;
  }
  if (isPluralPart(part)) {
    return `{{${[part.z, part.o, part.t, part.f, part.m, part.r].filter((value) => value !== void 0).join("|")}}}`;
  }
  return `{${part.k}${part.n ? "?" : ""}${part.f?.length ? `|${part.f.join("|")}` : ""}}`;
};

// ../generator/src/files/generate-types/parse-dictionary.mts
var parseDictionary = (translations, logger3, parentKeys = []) => (0, import_typesafe_utils8.isObject)(translations) ? Object.entries(translations).map(([key, text]) => {
  if ((0, import_typesafe_utils8.isString)(text)) {
...
var parseTypesFromSwitchCaseStatement = (formatters) => {
  ...
  if (!formatter.startsWith("{"))
    return void 0;
  const keys = formatter.replace(REGEX_BRACKETS, "").split(",").map((s) => s.split(":")).map(([key]) => key?.trim()).sort();
...
var parseTranslationEntry = ([key, text], logger3, parentKeys) => {
  ...
  const parsedObjects = parsedParts.filter(import_typesafe_utils8.isObject);
  const argumentParts = parsedObjects.filter((0, import_typesafe_utils8.not)(isPluralPart));
  const pluralParts = parsedObjects.filter(isPluralPart);
  const args = [];
  const types = {};
  argumentParts.forEach(({ k, n, i, f }) => {
    args.push({ key: k, formatters: f, optional: n });
    const type2 = i ?? parseTypesFromSwitchCaseStatement(f);
    types[k] = {
      types: (0, import_typesafe_utils8.uniqueArray)([...types[k]?.types || [], type2]).filter(import_typesafe_utils8.isNotUndefined),
  ...
  pluralParts.forEach(({ k }) => {
    if (!types[k]?.types.length) {
      ...
      if (!args.find(({ key: key2 }) => key2 === k)) {
        args.push({ key: k, formatters: [], pluralOnly: true });
      }
    }
  });
  Object.keys(types).forEach((key2) => {
    if (!types[key2]?.types.length) {
      types[key2] = { types: ["unknown"], optional: types[key2]?.optional };
    }
  });
  args.sort((0, import_typesafe_utils8.sortStringPropertyASC)("key"));
  const isValid = validateTranslation(key, types, logger3);
  if (!isValid)
    return null;
...
var validateTranslation = (key, types, logger3) => {
  ...
  const argKeys = Object.keys(types).sort(import_typesafe_utils8.sortStringASC);
  if ((0, import_typesafe_utils8.isArrayNotEmpty)(argKeys) && !isNaN(+argKeys[0])) {
    let expectedKey = "0";
...
var createJsDocsString = ({ text, types, pluralOnlyArgs } = {}, renderTypes = false, renderPluralOnlyArgs = true, renderNonBreakingSpaceBetweenCharacters = false) => {
  const renderedTypes = renderTypes ? `${Object.entries(types || {}).filter(({ "0": key }) => renderPluralOnlyArgs || !pluralOnlyArgs.includes(key)).sort((0, import_typesafe_utils9.sortStringPropertyASC)("0")).map(createJsDocsParamString).join("")}` : "";
...
var mapTranslationFunctions = (args = [], types) => {
  if (!args.length)
    return "";
  const uniqueArgs = args.filter((0, import_typesafe_utils10.filterDuplicatesByKey)("key"));
...
var createTranslationType = (parsedTranslations, jsDocInfo, nameOfType, namespaces = []) => {
  ...
  const namespaceTranslationsTypes = parsedNamespaceTranslations.map((value) => {
    ...
    if (!(0, import_typesafe_utils11.isArray)(parsedTranslations2))
      return "";
    return "export " + createTranslationType(
      (0, import_typesafe_utils11.isArray)(parsedTranslations2) ? parsedTranslations2 : [parsedTranslations2],
      jsDocInfo,
...
var getFormatterType = (formatter) => {
  if (!formatter.startsWith("{"))
    return formatter;
  const cases = formatter.replace(REGEX_BRACKETS, "").split(",").map((part) => part.split(":")).map(([key]) => [key, `\${string}`]).map((part) => part.join(":")).join(",");
  return `{${cases}}`;
};
var generateTranslationType = (args = []) => {
  if (!supportsTemplateLiteralTypes)
    return "string";
  const argStrings = args.filter((0, import_typesafe_utils11.isPropertyFalsy)("pluralOnly")).map(
    ({ key, optional, formatters }) => partAsStringWithoutTypes({ k: key, n: optional, f: formatters?.map(getFormatterType) }).replace(
      REGEX_BRACKETS,
      ""
    )
  );
  ...
  if (!containsRequiredParams)
    permutations.push([]);
  return permutations.map(getParamsType).filter(import_typesafe_utils11.filterDuplicates).join(PIPE_SEPARATION);
};
...
var createJsDocsMapping = (parsedTranslations) => {
  ...
  flattenToParsedResultEntry(parsedTranslations).forEach((parsedResultEntry) => {
    ...
    map[nestedKey] = {
      ...
      types,
      pluralOnlyArgs: args.filter((0, import_typesafe_utils12.isPropertyTrue)("pluralOnly")).map(({ key: key2 }) => key2)
    };
  });
  return map;
};
...
var getTypes = ({ translations, baseLocale, locales, typesTemplateFileName, banner, namespaces }, logger3) => {
  ...
  const parsedTranslations = parseDictionary(translations, logger3).filter(import_typesafe_utils12.isTruthy);
...
var getNamespaceLoaderForLocale = (locale, namespaces) => {
  return `${wrapObjectKeyIfNeeded(locale)}: {
	${namespaces.map((namespace) => `	${wrapObjectKeyIfNeeded(namespace)}: () => import('./${locale}/${namespace}')`).join(NEW_LINE_INDENTED_WITH_COMMA)}
	}`;
};
...
var aa = (sanitizedLocale, namespaces) => `{
...
var getLocalesTranslationRowSync = (locale, namespaces) => {
  ...
  const postfix = namespaces.length ? `: ${aa(sanitizedLocale, namespaces)}` : needsEscaping ? `: ${sanitizedLocale}` : "";
  return `
...
`);
  ...
  const outputPathParts = resolve3(outputPath, locale).replace(resolve3(), "").split(sep).filter(import_typesafe_utils13.isTruthy);
...
var runCommandAfterGenerator = (runAfterGenerator) => {
  logger2.info(`running command '${runAfterGenerator}'`);
...
// ../../node_modules/.pnpm/just-diff@5.1.1/node_modules/just-diff/index.mjs
...
// ../../node_modules/.pnpm/just-diff-apply@5.4.1/node_modules/just-diff-apply/index.mjs
var REMOVE = "remove";
var REPLACE = "replace";
var ADD = "add";
var MOVE = "move";
...
// src/setup/package-json.mts
import { execSync as execSync2 } from "child_process";
import { resolve as resolve5 } from "path";
var packageJsonPath = resolve5("package.json");
var pck = void 0;
var readPackageJson = async () => pck || (pck = await importFile(packageJsonPath, false));
var isEsmProject = async () => {
  const pck2 = await readPackageJson();
  return pck2?.type === "module";
};
var getDependencyList = async () => {
  const pck2 = await readPackageJson();
  const dependencyList = Object.keys(pck2?.dependencies || {});
  const devDependencyList = Object.keys(pck2?.devDependencies || {});
  const peerDependencyList = Object.keys(pck2?.peerDependencies || {});
  return [...dependencyList, ...devDependencyList, ...peerDependencyList];
};
var npmLockPath = resolve5("package-lock.json");
var yarnLockPath = resolve5("yarn.lock");
var pnpmLockPath = resolve5("pnpm-lock.yaml");
...
var getDefaultConfig = async () => {
  ...
  const isTypeScriptProject = dependencies.includes("typescript") || await doesPathExist(resolve6("tsconfig.json"));
...
var getConfigDiff = async (options) => {
  ...
  return Object.fromEntries(Object.entries(changedValues).filter((0, import_typesafe_utils14.isPropertyNotUndefined)("1")));
};
